...
export function Banner({ children, className, type }:
  BannerProps
) {
  ...
  const typeIcon = (type: BannerType) => {
    switch (type) {
      case 'warning':
        return (
          <div className="w-4 h-4 shrink-0 fill-current opacity-80 mt-[3px] mr-3">
            <ReactAnimatedIcon size="small" iconName="NOTIFICATION" categoryName="USER_INTERFACE" />
          </div>
        );
      case 'error':
        return (
  ...
  const typeColor = (type: BannerType) => {
    switch (type) {
      case 'warning':
        return 'bg-red-500';
      case 'error':
